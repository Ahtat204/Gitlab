...
class ProjectRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAllProjects(): Flow<GetMyProjectsPaginatedQuery.Data> =
        apolloClient.
        query(GetMyProjectsPaginatedQuery()).fetchPolicy(FetchPolicy.CacheFirst).
        watch().
    ...
    override suspend fun getCurrentUserWorkItems(cursor: String?): Flow<GetCurrentUserWorkItemsQuery.Data> {
        return if (cursor == null) apolloClient.query(GetCurrentUserWorkItemsQuery()).fetchPolicy(
        ...
        else apolloClient.query(GetCurrentUserWorkItemsQuery(Optional.present(cursor))).fetchPolicy(
            FetchPolicy.CacheFirst
        ).watch().map { response ->
            response.exception?.cause?.let {
                throw it
    ...
    override suspend fun getRepositoryBranches(
        ...
        return apolloClient.
        query(GetRepositoryBranchesQuery(project, skip))
    ...
    override suspend fun getProjectRepository(
        ...
        return if (branch == null) {
            if (path != null) {
                apolloClient.
                query(GetProjectRepositoryQuery(id, path = Optional.present(path)))
                    .fetchPolicy(FetchPolicy.CacheFirst).
                    watch().
                    mapNotNull { it.data }
                    .catch { ex ->
                ...
                apolloClient.
                query(GetProjectRepositoryQuery(id))
                    .fetchPolicy(FetchPolicy.CacheFirst).
                    watch().
                    mapNotNull { it.data }
                    .catch { ex ->
